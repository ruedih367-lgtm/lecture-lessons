    ...
    <div style={styles.container}>
      <div style={styles.content}>
        <div style={styles.header}>
          <Link href={`/lectures/${lectureId}`} style={styles.backLink}>← Back to Lecture</Link>
          {messages.length > 0 && (
        ...
        <div style={styles.titleCard}>
          ...
          {learningStatus?.status === 'learning' && (
            <div style={styles.statusBadgeLearning}>🧠 {learningStatus.message}</div>
          )}
          {!isAuthenticated && (
        ...
        <div style={styles.modeSelector}>
          {['tutor', 'practice', 'exam'].map((m) => (
            ...
            >
              {m === 'tutor' ? '💬 Tutor' : m === 'practice' ? '📝 Practice' : '📋 Exam'}
            </button>
          ))}
        </div>
        ...
        <div style={styles.messagesContainer}>
          {messages.length === 0 && (
            <div style={styles.emptyState}>
              ...
              <p style={styles.tableTip}>💡 <strong>Tip:</strong> Ask for comparisons or data in table format!</p>
              {isAuthenticated && (
                <p style={styles.likeHint}>
                  👍 Like helpful responses to train the AI to your learning style!
                </p>
              )}
            </div>
          )}
